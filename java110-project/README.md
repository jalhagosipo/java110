...
- 작업
    ...
    - Spring 트랜잭션 관리자를 Java Config로 등록헌더,
    - 트랜잭션 관리자를 활성화하는 애노테이션을 Java Config에 설정한다.
       @EnableTransactionManagement 애노테이션 추가한다.
    - 트랜잭션으로 묶어서 DB SQL을 실행해야 하는 메서드에 @Transactional 애노테이션을
    붙인다.
